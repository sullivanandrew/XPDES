...
//:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
	...
	for (i = 0; i < m-1; ++i)
  {
	...
	{
		{
			...
			dM = dMcoeff*zeta;
			//dJ = dJcoeff*zeta;
			dJ = Minf*sqrt(pow(Minf,2.0)-4.0*pow(r_H,2.0));
	...
	sprintf(foutname,"./Data/PlotData/");
	strcat(foutname, SolType);
	char buffer[256];
	sprintf(buffer,"prop_tol%03i_rH%03i.dat",(int) round(-log10(tol)), (int) round(r_H*1.0E2));
	...
	fprintf(foutfile,"%.3f %.2f %.1f %.4f %14.7e %14.7e ", alpha, r_H, chi, Omega_H, M, J0);
	fprintf(foutfile,"%14.7e %14.7e %14.7e ",	Minf, Minf/M-1.0, dMcoeff*zeta);
	...
	fprintf(foutfile,"%14.7e %14.7e %14.7e\n", xLR, xLR/xLRKERRsol-1.0, dxLRcoeff*zeta);
...
/*
	int check;
	char dirname[256];
	sprintf(dirname,"./Data/PlotData/angleplotdata_al%06i_chi%03i_n%03i_m%02i", (int) round(alpha*1.0E4), (int) round(chi*1.0E2), n, m);
	...
	strcat(pkoutname, "/sGBfromKERR.dat");
	...
	for (j = 0; j < n; ++j)
	{
		...
		for (i = 0; i < 3; ++i)
		{
			yk = y[yplot[i]];
			k = (yplot[i]*n+j)*p;
			fprintf(pkoutfile," %22.15e %22.15e %22.15e %22.15e %22.15e", Psi[k +0] - pow(xk,2.0)*IC0out(xk,yk,r_H,M,0.0,1.0,0.0),
			Psi[k +1] - pow(xk,2.0)*IC1out(xk,yk,r_H,M,0.0,1.0,0.0),
			Psi[k +2] - pow(xk,2.0)*IC2out(xk,yk,r_H,M,0.0,1.0,0.0),
			Psi[k +3] - IC3out(xk,yk,r_H,M,0.0,1.0,0.0),
			Psi[k +4] - IC4out(xk,yk,r_H,M,0.0,1.0,0.0));
		}
		fprintf(pkoutfile,"\n");
	}
	fclose(pkoutfile);
	...
	strcat(psoutname, "/sGBfromlinsGB.dat");
