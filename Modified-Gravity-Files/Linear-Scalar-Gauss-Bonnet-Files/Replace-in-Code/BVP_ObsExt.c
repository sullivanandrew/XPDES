...
int main(int argc, char *argv[])
{
	...
	char InputName[256];
  strcpy(InputName, "./Data/BVPout/BVPout_sols/");
  strcat(InputName, argv[1]);
  strcat(InputName, ".dat");
  printf("File name: %s\n",InputName);

  const int p = atoi(argv[2]); //Number of field equations. Will always be twice the number of fields due to mixed derivatives
	const int n = atoi(argv[3]); //x grid size
	const int m = atoi(argv[4]); //theta grid size
	const int r = atoi(argv[5]); //Newton polynomial order
	const double tol = pow(10.0,atof(argv[6])); //absolute tolerance of solver
	const double alpha = atof(argv[7]); //coupling parameter alpha
	const double beta = 1.0; //coupling beta set to 1
	const double r_H = atof(argv[8]); //rH value. Note: this is technically \bar{r}_H = r_H/4 <- Schwarzschild
	...
	{
		{
			...
			printf("gtt = %14.7e. gtph = %14.7e. gpsi = %14.7e.\n",gtt,gtph,gpsi);
			...
			printf("compared to LIN solution dM = zeta*49/640 = %12.5e.\n", dM);
	...
	{
		{
			...
			double J2,Mf2,Mm2,D2,a2;
			double m2rem, l2rem;
			double chi2;

			//Mass and Charge calc
			J2 = u3d00k*pow(rhoT,2.0)/2.0;
			D2 = u4d00k*rhoT;
			Mf2 = rhoT/2.0*(1.0 - sqrt(2.0*u0d00k - 1.0));
			a2 = J2/Mf;
			chi2 = a2/Mf;
			l2rem = u2d00k - (1.0 + 2.0*Mf/rhoT + (-pow(D2,2.0)/4.0 + 2.0*pow(Mf,2.0) - 2.0*pow(r_H,2.0))/pow(rhoT,2.0));
			m2rem = u1d00k - (1.0 + 2.0*Mf/rhoT + (-pow(D2,2.0)/4.0 + pow(Mf,2.0) + 2.0*pow(r_H,2.0) + pow(a2,2.0)*pow(cos(yk),2.0))/pow(rhoT,2.0));
			printf("J   = %14.7e, with an error of O(1/rho^2) = %11.4e.\n",J2,1.0/pow(rhoT,2.0));
			printf("D2  = %14.7e.\n",D2);
			printf("Mf2 = %14.7e.\n",Mf2);
			printf("a2  = %14.7e. chi2 = %14.7e.\n",a2,a2/Mf);
			printf("m2rem = %14.7e, l2rem = %14.7e.\n",m2rem,l2rem);
...
*/
	...
	//-------------------- ISCO & LR variables --------------------
	xISCOSCHW = sqrt(6.0)/3.0;
	xLRSCHW = sqrt(3.0)/3.0;
	rhoISCOSCHW = r_H*(1.0+xISCOSCHW)/(1.0-xISCOSCHW);
	rhoLRSCHW = r_H*(1.0+xLRSCHW)/(1.0-xLRSCHW);

	double Z1, Z2;
	Z1 = 1.0 + cbrt(1.0-pow(chi,2.0))*(cbrt(1.0+chi) + cbrt(1.0-chi));
	Z2 = sqrt(3.0*pow(chi,2.0) + pow(Z1,2.0));
	printf("Z1 = %f. Z2 = %f.\n",Z1,Z2);
	rISCOKERR = M*(3.0 + Z2 - sqrt((3.0-Z1)*(3.0 + Z1 + 2.0*Z2)));
	rhoISCOKERR = -M/2.0 + rISCOKERR/2.0 + sqrt(pow(rISCOKERR,2.0) - 2.0*M*rISCOKERR + pow(chi*M,2.0))/2.0;
	xISCOKERR = (1.0 - r_H/rhoISCOKERR)/(1.0 + r_H/rhoISCOKERR);
	printf("rISCOKERR = %f [M]. rhoISCOKERR = %f [M]. xISCOKERR = %f.\n",rISCOKERR,rhoISCOKERR,xISCOKERR);
	rLRKERR = 2.0*M*(1.0 + cos(2.0/3.0*acos(chi)));
	rhoLRKERR = -M/2.0 + rLRKERR/2.0 + sqrt(pow(rLRKERR,2.0) - 2.0*M*rLRKERR + pow(chi*M,2.0))/2.0;
	xLRKERR = (1.0 - r_H/rhoLRKERR)/(1.0 + r_H/rhoLRKERR);
	printf("rLRKERR   = %f [M]. rhoLRKERR   = %f [M]. xLRKERR   = %f.\n",rLRKERR,rhoLRKERR,xLRKERR);
