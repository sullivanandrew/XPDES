...
int BVP_out(struct param_type *params, double x[], double y[], double Psi[], double JacVal[], int JacRow[], int JacCol[], double dPsi[], double b[], double Dx[], double Dy[], int it)
{
	...
	{
		sprintf(poutname,"./Data/BVPout/BVPout_sols/sol_al%06i_tol%03i_it%02i_rH%03i_chi%03i_n%02i_m%02i.dat",(int) round(alpha*1.0E4), (int) round(-log10(tol)), it, (int) round(r_H*1.0E2), (int) round(chi*1.0E2), n, m);
	}
	else
	{
		sprintf(poutname,"./Data/BVPout/BVPout_iterations/sol_al%06i_tol%03i_it%02i_rH%03i_chi%03i_n%02i_m%02i.dat",(int) round(alpha*1.0E4), (int) round(-log10(tol)), it, (int) round(r_H*1.0E2), (int) round(chi*1.0E2), n, m);
	}
