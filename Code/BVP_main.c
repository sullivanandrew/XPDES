...
int BVP_main(const int plotvar, const int linsolvar, const int r, int n, int m, const int p, int N, const double tol, const double chi, const double ICdiff, const double r_H, const double M, const double alpha, const double beta)
{
	//-------------------- Initialization --------------------
	...
	struct ICparam_type IC_params; //Initial condition parameters
	IC_params.r_HICparam = r_H;
	double MIC = 2.0*r_H/sqrt(1.0-pow(chi,2.0));
	IC_params.MICparam = MIC; //M;
	printf("ICdiff = %.5f. Mass of initial condition = %.2f.\n",ICdiff,MIC);
	IC_params.alphaICparam = alpha;
	IC_params.betaICparam = beta;
	IC_params.deltaICparam = ICdiff;
	...
	//Initial Conditions
	status = BVP_GENIC(&my_params, &IC_params, x, y, Psi, GRIDdx, GRIDdy); //Calculate initial condition
	//status = BVP_out(&my_params, x, y, Psi, JacVal, JacRow, JacCol, dPsi, b, Dx, Dy, 0); //Initial Conditions Output
